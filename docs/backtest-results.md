...
## 2026-08-08 Update — 新アルゴリズム3種の検証（タートル / TSMOM / BBスクイーズ）

既存バリアントはすべて「ドンチャン or MAクロスで入り、ATR/Supertrend ストップで出る」系統
だったため、**構造的に異なる**3方式を実装して比較した。

| 方式 | 何が違うか |
|---|---|
| **タートル式ドンチャン・エグジット** (`donchian_exit_term`) | 決済を MA クロス（遅行）ではなく N バー逆方向チャネル割れで行う |
| **時系列モメンタム (TSMOM)** (`mom_lookback`) | MA200 の代わりに N バー騰落率でレジーム判定 |
| **ボリンジャー・スクイーズ** (`bb_squeeze`) | バンド幅が低位25%（ボラ収縮）のときのみエントリー |

### 結論: 明確な「新王者」はなし。ただし**決済ロジックに региме依存の強い効果**を確認

4つの検証窓での Calmar（リターン% ÷ 最大DD%）順位:

| バリアント | full 2017–2026 | test>2020 | test>2022 | test>2024 | 平均順位 |
|---|---:|---:|---:|---:|---:|
| `TURTLE_EXIT20_STOP1.0_GARCH40` | **1** | **1** | 2 | 6 | **2.50** |
| `BREAKOUT_STOP1.0_GARCH40`（現行推奨） | 2 | 2 | 3.5 | 3.5 | 2.75 |
| `TURTLE_EXIT10_STOP1.0_GARCH40` | 5 | 6 | **1** | **1** | 3.25 |
| `MOM180_BREAKOUT_STOP1.0_GARCH40` | 6 | 3 | 3.5 | 2 | 3.62 |
| `MOM90_MA200_BREAKOUT_STOP1.0_GARCH40` | 4 | 5 | 6 | 5 | 5.00 |
| `BREAKOUT_SUPERTREND4_GARCH40` | 3 | 4 | **7** | **7** | 5.25 |

### 主要な発見

**1. 10バー・ドンチャン決済は近年のレジームで圧倒的、しかし過去では劣る**

`TURTLE_EXIT10_STOP1.0_GARCH40` は直近2窓で1位。とくにドローダウンの改善が大きい:

| 窓 | リターン | 最大DD | Calmar |
|---|---:|---:|---:|
| test>2022 — 現行推奨 | +177.6% | 24.7% | 7.18 |
| test>2022 — **EXIT10** | **+203.9%** | **18.0%** | **11.31** |
| test>2024 — 現行推奨 | +53.7% | 24.8% | 2.17 |
| test>2024 — **EXIT10** | **+70.8%** | **16.5%** | **4.30** |

一方 full 2017–2026 では +913% と、現行推奨の +1,405% に大きく劣る（順位5位）。
**解釈**: 10バー決済は利を早めに確定するため、2017–2021 の超巨大トレンドでは利益を取り逃す。
逆に 2022 年以降の往来相場では利益を守る側に働く。決済の速さがレジームと直結している。

**2. 20バー決済なら全窓で安定**

`TURTLE_EXIT20_STOP1.0_GARCH40` は平均順位トップ。ただし現行推奨との差は僅差
（full: Calmar 48.13 vs 47.85）で、置き換えを正当化するほどの優位はない。

**3. TSMOM は MA200 の代替として概ね中立**

`MOM180` は近年やや良いが一貫した優位なし。`MOM90` + MA200 の併用はフィルターが過剰で
トレード数が減り、かえって悪化。

**4. ボリンジャー・スクイーズは BTC では機能しない**

`BBSQUEEZE_BREAKOUT_STOP1.0_GARCH40` は full 期間で +15% (n=11)、PF 1.62 と最下位級。
BTC の大相場は「ボラが既に高い状態」から始まることが多く、収縮待ちでは主要トレンドを
すべて取り逃す。**この方向は追わなくてよい。**

### グリッドサーチ: ストップ幅 × 決済期間（18通り × 4窓）

ドンチャンブレイクアウト + MA200 + GARCH40 を固定し、ストップ幅（0/0.5/1.0/1.5/2.0/3.0 ATR）
と決済方式（MAクロス / 10バー / 20バー）を総当たり。Calmar 順位の平均で評価:

| 組み合わせ | full | >2020 | >2022 | >2024 | 平均順位 |
|---|---:|---:|---:|---:|---:|
| `stop1.0 / MAクロス`（現行推奨） | 2 | 2 | 4 | 3.5 | **2.88** |
| `stop1.0 / exit20` | **1** | **1** | 3 | 7 | **3.00** |
| `stop1.0 / exit10` | 10 | 13 | **1** | **1** | 6.25 |
| `stop1.5 / exit10` | 5 | 10 | 2 | 2 | 4.75 |
| `stop0.5 / *` | 12–18 | 9–18 | 12–17 | 10–18 | 14.0+ |
| `stop3.0 / MAクロス` | 14 | 14 | 18 | 17 | 15.75 |

**発見1: 1.0×ATR ストップは決済方式に関係なく最良 — 現行設定は過学習ではない**

上位4組はすべて stop1.0 または stop1.5。0.5 は狭すぎて刈られ、2.0〜3.0 は広すぎて損失が膨らむ。
ストップ幅の最適点は決済ロジックから独立しており、既存のグリッドサーチ結果を追試の形で裏付けた。

**発見2: 10バー決済は「リターンを削ってDDを買う」ダイヤル**

同じストップ幅で決済方式だけを変えた最大DDの比較（全窓）:

| ストップ | MAクロス | exit10 |
|---|---:|---:|
| 1.0 ATR | 24.7–29.4% | **16.5–30.5%** |
| 1.5 ATR | 30.0–30.1% | **22.1–23.4%** |
| 2.0 ATR | 34.6–34.7% | **25.8–27.4%** |
| 3.0 ATR | 39.9–40.1% | **26.1–27.4%** |

exit10 はどのストップ幅でも一貫してDDを下げる（レジーム非依存の効果）。
一方リターンは下がるため、順位が窓によって入れ替わるのはこのトレードオフの現れ。

### 注意（過学習リスク）と結論

直近窓の1位を「新ベスト」と宣言するのは選択バイアスになる（12バリアント + 18組を試している）。
**現行推奨 `BREAKOUT_STOP1.0_GARCH40` は据え置き**。判断材料として:

- **リターン最大化 / 全期間で頑健** → 現行の `stop1.0 / MAクロス` のまま
- **DD を抑えたい・近年のレジームに賭ける** → `TURTLE_EXIT10_STOP1.0_GARCH40`
- **両者の中間** → `stop1.5 / exit10`（平均順位4.75、全窓で中位以上と最も安定）
